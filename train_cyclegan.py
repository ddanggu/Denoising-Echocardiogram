"""
Supervised training script for paired CycleGAN dehazing.
"""
...
def get_opt():
    ...
    p.add_argument("--checkpoints_dir", type=str, default="./checkpoints_cyclegan")
...
def save_state(tag, model, optimizers, epoch, metrics, checkpoints_dir):
    ...
    save_checkpoint(ckpt_dir / f"{tag}_net_SS_G_XtoY.pth", model.G_XtoY, {"G": optimizers["G"]}, epoch, metrics)
    save_checkpoint(ckpt_dir / f"{tag}_net_SS_G_YtoX.pth", model.G_YtoX, {"G": optimizers["G"]}, epoch, metrics)
    save_checkpoint(ckpt_dir / f"{tag}_net_SS_D_X.pth", model.D_X, {"D": optimizers["D"]}, epoch, metrics)
    save_checkpoint(ckpt_dir / f"{tag}_net_SS_D_Y.pth", model.D_Y, {"D": optimizers["D"]}, epoch, metrics)
...
def train():
    ...
    model = CycleGAN(opt.in_ch, opt.out_ch, opt.G_init_ch, opt.D_init_ch, opt.depth, opt.out_act, opt.norm).to(device)
    optimizer_G = torch.optim.Adam(
        list(model.G_XtoY.parameters()) + list(model.G_YtoX.parameters()),
        lr=opt.lr_G,
        betas=(opt.beta1, opt.beta2),
    )
    optimizer_D = torch.optim.Adam(
        list(model.D_X.parameters()) + list(model.D_Y.parameters()),
        lr=opt.lr_D,
        betas=(opt.beta1, opt.beta2),
    )
    ...
    for epoch in range(start_epoch, opt.n_epochs + 1):
        ...
        for img, label, _ in train_loader:
            ...
            fake_y = model.G_XtoY(img)
            fake_x = model.G_YtoX(label)

            loss_D_Y_real = gan_loss(model.D_Y(label), True)
            loss_D_Y_fake = gan_loss(model.D_Y(fake_y.detach()), False)
            loss_D_X_real = gan_loss(model.D_X(img), True)
            loss_D_X_fake = gan_loss(model.D_X(fake_x.detach()), False)
            loss_D = 0.5 * (loss_D_Y_real + loss_D_Y_fake + loss_D_X_real + loss_D_X_fake)
            ...
            fake_y = model.G_XtoY(img)
            fake_x = model.G_YtoX(label)
            loss_G_gan = (gan_loss(model.D_Y(fake_y), True) + gan_loss(model.D_X(fake_x), True)) * opt.lambda_gan
            loss_G_pair = (pair_loss(fake_y, label) + pair_loss(fake_x, img)) * opt.lambda_pair
            loss_G_ssim = fake_y.new_tensor(0.0)
            if ssim_loss is not None:
                loss_G_ssim = (
                    ssim_loss(fake_y * 0.5 + 0.5, label * 0.5 + 0.5)
                    + ssim_loss(fake_x * 0.5 + 0.5, img * 0.5 + 0.5)
                ) * opt.lambda_ssim
            loss_G_edge = fake_y.new_tensor(0.0)
            if edge_loss is not None:
                loss_G_edge = (
                    edge_loss(fake_y * 0.5 + 0.5, label * 0.5 + 0.5)
                    + edge_loss(fake_x * 0.5 + 0.5, img * 0.5 + 0.5)
                ) * opt.lambda_edge
    ...
    print(f"Best checkpoint: {Path(opt.checkpoints_dir) / 'best_net_SS_G_XtoY.pth'}")
